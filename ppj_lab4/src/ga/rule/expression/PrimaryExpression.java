...
public class PrimaryExpression extends RuleStrategy {
    ...
    public void emit(NonTerminalNode node, Environment environment) {
        ...
        if (childrenNumber == 1) {
            ...
            if (childNode.getSymbol().getSymbol().equals("IDN")) {
                if(Ulaz.functionCall) {
                    System.out.println("\t CALL F_" + childNode.getValue().toUpperCase());
                }  else {
                    String id = childNode.getValue();
                    if(DeclarationList.params != null) {
                        int offset = environment.findLocalOffset(id) - 4;
                        String hex = Integer.toHexString(offset);
                        System.out.println("\t LOAD R0, (R7 + 0" + hex + ")");
                    } else if(environment.isDeclaredLocaly(id)) {
                        int offset = environment.findLocalOffset(id);
                        String hex = Integer.toHexString(offset);
                        if(!JoiningExpression.left) {
                            System.out.println("\t LOAD R0, (R5 + 0" + hex + ")");
                        } else {
                            System.out.println("\t POP R0");
                            System.out.println("\t STORE R0, (R5 + 0" + hex + ")");
                        }
                    } else if (environment.isParameter(id)) {
                        //TODO local offset
                        int offset = environment.parameterOffset(id);
                        String hex = Integer.toHexString(offset);
                        if(!JoiningExpression.left) {
                            System.out.println("\t LOAD R0, (R5 + 0" + hex + ")");
                        } else {
                        ...
                        if(!JoiningExpression.left) {
                            ...
                            System.out.println("\t POP R0");
                            System.out.println("\t STORE R0, (G_" + id.toUpperCase() + ")");
                        }
                    } else {
                ...
                Ulaz.konst = new int[] { broj };
                if (!Ulaz.init) {
                    if (broj < 500_000) {
                        System.out.println("\t MOVE %D " + num + ", R0");
                        System.out.println("\t PUSH R0");
                    } else {
                        int loc = Ulaz.addBig(broj);
                        System.out.println("\t LOAD R0, (VEL" + loc + ")");
                        System.out.println("\t PUSH R0");
                    }
                }
            } else if (childNode.getSymbol().getSymbol().equals("ZNAK")) {
                int ascii = (int) childNode.getValue().charAt(1);
                Ulaz.konst = new int[] { ascii };
                if (!Ulaz.init) {
